...
def test_apply_dnsmasq_defaults_to_dry_run(monkeypatch):
    ...
    monkeypatch.setattr(main, "build_dnsmasq_apply", lambda cfg, apply_changes=False: seen.append(apply_changes) or _envelope())
...
def test_apply_dnsmasq_json_is_one_envelope(monkeypatch):
    monkeypatch.setattr(main, "_load_config", lambda path: object())
    monkeypatch.setattr(main, "build_dnsmasq_apply", lambda cfg, apply_changes=False: _envelope())
...
def test_apply_dnsmasq_failure_exits_one(monkeypatch):
    monkeypatch.setattr(main, "_load_config", lambda path: object())
    monkeypatch.setattr(main, "build_dnsmasq_apply", lambda cfg, apply_changes=False: _envelope(ok=False))

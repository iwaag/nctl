...
def test_destroy_gate_rejections_are_named():
    ...
    cases = [
        ("instance_kind_not_container", lambda s: s.desired.compute_instances.__setitem__(0, s.desired.compute_instances[0].model_copy(update={"instance_kind": "virtual_machine"}))),
        ("guest_type_not_lxc", lambda s: s.actual.virtual_machines.__setitem__(0, s.actual.virtual_machines[0].model_copy(update={"proxmox": s.actual.virtual_machines[0].proxmox.model_copy(update={"guest_type": "qemu"})}))),

...
def test_dnsmasq_config_action_with_blocked_render_never_invokes_ansible(tmp_path, monkeypatch):
    """VM p3 Step 6 defense in depth: `executor.py`'s `dnsmasq_config` branch
    calls `build_dnsmasq_apply()` fresh at execution time (Corrected contract
    3-adjacent behavior, unchanged by this step) -- when the render underneath
    it is blocked (a `desired_mac_mismatch` fixture), the action must fail
    ...
    dnsmasq_action = ReconcileAction(
        ...
        action_kind="dnsmasq_config",
        targets=[Target(kind="service", slug="dnsmasq", name="dnsmasq", id="svc-1")],
        claimed_diff_codes=["desired_mac_mismatch"],
        reason="test",
        mutates=True,
        # No post-actuation observation here on purpose -- that is a separate
        # ansible-based inventory-collection path, unrelated to this test's
        # claim about the dnsmasq deploy playbook specifically.
        requires_observation=False,
        parameters={"host_slugs": [ctx.node.slug]},
    )
    plan = ctx.make_plan([dnsmasq_action])
    ...
    def _blocked_render(cfg, operation_id=None):
        return Envelope.build(
            "nctl.render.dnsmasq.v3",
            _blocked_dnsmasq_render_data(),
            [EnvelopeError(code="desired_mac_mismatch", message="edge-1/primary: desired_mac_mismatch blocks the shared dnsmasq render")],
        )

    monkeypatch.setattr("nctl_core.dnsmasq_apply.build_dnsmasq_render", _blocked_render)

    ansible_calls = []
    monkeypatch.setattr(
        "nctl_core.ansible._run_command",
        lambda *a, **kw: ansible_calls.append((a, kw)) or (_ for _ in ()).throw(AssertionError("ansible must never run")),
    )
    ...
    dnsmasq_result = next(a for a in outcome.summary.actions if a.reconciler_id == "dnsmasq_config")
    assert dnsmasq_result.success is False
    assert ansible_calls == []


def _blocked_dnsmasq_render_data():
    from nctl_core.dnsmasq_render import DnsmasqRenderData

    return DnsmasqRenderData(
        schema_version="5.0",

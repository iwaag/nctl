...
- **JSON output**: every command will return a stable `nctl.<command>.v1` envelope via `--json`
  (spec lands in `docs/output-format.md`, Step 0.4).
- **Event logs**: long-running operations emit JSON Lines with an operation ID
  (spec lands in `docs/event-log.md`, Step 0.5).

...
summary, and execution mode (`plan` or `apply`). A `plan` renders the artifact and resolves its
target hosts without invoking SSH or Ansible. An `apply` additionally records SSH preflight
entries, the daemon `setup` result, and the records-deploy Ansible command's exit code, stdout,
stderr, and parsed per-host recap.
